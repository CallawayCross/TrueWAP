...
```{r plot_rolling, eval=TRUE}
xts.nikkei.ohlcv <- xts(nikkei[c("Open", "High", "Low", "Close", "Volume")], order.by=as.Date(nikkei$Date))
xts.twap <- xts(twap, order.by=as.Date(nikkei$Date))
xts.vwap <- xts(vwap, order.by=as.Date(nikkei$Date))
xts.true_wap <- xts(true_wap, order.by=as.Date(nikkei$Date))

chartSeries(xts.nikkei.ohlcv, theme = chartTheme('white'), type = "candlesticks", subset = "last 27 months", name = "Nikkei 225 (Osaka), Active Daily Continuation", minor.ticks = TRUE)
addTA(xts.twap, col = "blue", lty = 1, lwd = 3, on = 1)
addTA(xts.vwap, col = "red", lty = 1, lwd = 3, on = 1)
addTA(xts.true_wap, col = "darkgreen", lty = 1, lwd = 3, on = 1)
```
...
```{r plot_fixed, eval=TRUE}
xts.nikkei.ohlcv <- xts(nikkei[c("Open", "High", "Low", "Close", "Volume")], order.by=as.Date(nikkei$Date))
xts.twap_anchored <- xts(twap_anchored, order.by=as.Date(nikkei$Date))
xts.vwap_anchored <- xts(vwap_anchored, order.by=as.Date(nikkei$Date))
xts.true_wap_anchored <- xts(true_wap_anchored, order.by=as.Date(nikkei$Date))

chartSeries(xts.nikkei.ohlcv, theme = chartTheme('white'), type = "candlesticks", subset = "last 27 months", name = "Nikkei 225 (Osaka), Active Daily Continuation", minor.ticks = TRUE)
addTA(xts.twap_anchored, col = "blue", lty = 1, lwd = 3, on = 1)
addTA(xts.vwap_anchored, col = "red", lty = 1, lwd = 3, on = 1)
addTA(xts.true_wap_anchored, col = "darkgreen", lty = 1, lwd = 3, on = 1)
```
